...
p_obs = nan(length(b0B_all),length(b1_all),length(Vv_all),length(Vi_all),nsubj,6);
        ...
        for sx3 = 1:length(b1_all)
            ...
            for idx = 1:nsubj
                for n = 1:length(u) % looping over the trial list
                    ...
                    probs_1 =    RDM_pdf(1:3000,drift_r1(n),B_dx(n));
                    P1 = randsample(1:3000, 1, true,probs_1);
                    probs_2 =    RDM_pdf(1:3000,drift_r2(n),B_sx(n));
                    P2 = randsample(1:3000, 1, true,probs_2);
                    P = [P1 P2];
            ...
            parfor idx = 1:nsubj % fit the model on every simulated subject
                ...
                m = tapas_fitModel(y,...
                    ...
                    'RDM_hgf_config',...
                    'tapas_quasinewton_optim_config');
                p_obs(sx,sx,sx,sx2,idx,:)=m.p_obs.p;
                p_prc(sx,sx,sx,sx2,idx,:)=m.p_prc.om(2:3);
...
% Saving the outputs
save D:\PAM\PAM_02_08_2024\results\rdm_HGF_B_sim_results_time.mat models
