...
tot = length(b0B_all)*length(b1_all)*length(Vv_all)*length(Vi_all)*length(bv_all)*nsubj;
p_obs = nan(length(b0B_all),length(b1_all),length(Vv_all),length(Vi_all),length(bv_all),nsubj,7);
p_prc = nan(length(b0B_all),length(b1_all),length(Vv_all),length(Vi_all),length(bv_all),nsubj,2);
            ...
            for Vix = 1:length(Vi_all)
                for bvx = 1:length(bv_all)
                    ...
                    parfor idx = 1:nsubj % fit the model on every simulated subject
                        ...
                        temp_table.b0B = repmat(b0B_all(b0Bx), numel(idx), 1); % Replicate 'a' values for each subject
                        temp_table.b1 = repmat(b1_all(b1x), numel(idx), 1); % Replicate 'sigma' values for each subject
                        temp_table.Vv = repmat(Vv_all(Vvx), numel(idx), 1); % Replicate 'f' values for each subject
                        temp_table.Vi = repmat(Vi_all(Vix), numel(idx), 1); % Replicate 'f' values for each subject
                        temp_table.bv = repmat(bv_all(bvx), numel(idx), 1); % Replicate 'f' values for each subject
...
% Saving the outputs
save(fullfile('Results_RDM','SIM_drift.mat'),'p_obs','p_prc','b0B_all','b1_all',"Vv_all","Vi_all", "bv_all")
save(fullfile('Results_RDM','table_drift.mat'),'models')