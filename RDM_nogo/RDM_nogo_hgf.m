function [logp, yhat, res] = RDM_hgf_nogo(r, infStates, ptrans)
...
for ntrial = 1:length(u)
        ...
        if not(isnan(rt(ntrial)))
            prob = utl_inverse_gaussian_defective(rt(ntrial),drift_pdf,drift_cdf,B_pdf,B_cdf);
            prob = max(prob, 1e-32);
            logp_reg(ntrial) = log(prob);
        elseif isnan(rt(ntrial))
            fun = @(x) utl_inverse_gaussian_defective(x,drift_pdf,drift_cdf,B_pdf,B_cdf);
