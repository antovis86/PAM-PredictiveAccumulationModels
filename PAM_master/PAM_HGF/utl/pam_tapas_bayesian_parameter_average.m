function bpa = pam_tapas_bayesian_parameter_average(datasets)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
prc_model = datasets{1}.c_prc.model;
obs_model = datasets{1}.c_obs.model;

% Get priors
prc_priormus = datasets{1}.c_prc.priormus;
prc_priorsas = datasets{1}.c_prc.priorsas;
obs_priormus = datasets{1}.c_obs.priormus;
obs_priorsas = datasets{1}.c_obs.priorsas;

% Check whether everything matches up
for i = 2:n
    if ~strcmp(prc_model,datasets{i}.c_prc.model)
        error('tapas:hgf:bpa:PrcModNoMatch', 'Perceptual models do not match.');
    end

    if ~strcmp(obs_model,datasets{i}.c_obs.model)
        error('tapas:hgf:bpa:ObsModNoMatch', 'Observation models do not match.');
    end

    if ~isequalwithequalnans(prc_priormus,datasets{i}.c_prc.priormus) || ~isequalwithequalnans(prc_priorsas,datasets{i}.c_prc.priorsas)
        error('tapas:hgf:bpa:PrcPriorsNoMatch', 'Perceptual priors do not match.');
    end

    if ~isequalwithequalnans(obs_priormus,datasets{i}.c_obs.priormus) || ~isequalwithequalnans(obs_priorsas,datasets{i}.c_obs.priorsas)
        error('tapas:hgf:bpa:ObsPriorsNoMatch', 'Observation priors do not match.');
    end

    if ~isequalwithequalnans(u(:),datasets{i}.u(:))
...
bpa.u     = u;
bpa.ign   = [];
bpa.c_prc = datasets{1}.c_prc;
bpa.c_obs = datasets{1}.c_obs;
...
for i=1:n
    mui = [datasets{i}.p_prc.ptrans, datasets{i}.p_obs.ptrans]';
    mui = mui(opt_idx);
    mu = mu + datasets{i}.optim.H*mui;
end
