function c = PAM_rdm_vkf_fitModel(u,y,c)
% This is the main function for fitting the parameters of the "PAM_rdm_vkf"
% model
% --------------------------------------------------------------------------------------------------
...
% FIT MODEL
x0 = c.initpt(~c.fix);
fun = @(x) PAM_rdm_vkf(x,u,y,c);
options = optimoptions('fminunc', 'Algorithm', 'quasi-newton');
x = fminunc(fun, x0, options);

c.final = c.initpt;
c.final(~c.fix) = x;
c.final_transp = PAM_rdm_vkf_transp(c.final,y);

c.final_prc.lambda = c.final_transp(1);
c.final_prc.v0 = c.final_transp(2);
c.final_prc.omega = c.final_transp(3);

c.final_obs.a_a = c.final_transp(4);
c.final_obs.b_a = c.final_transp(5);
c.final_obs.a_v = c.final_transp(6);
c.final_obs.b_val = c.final_transp(7);
c.final_obs.b_v = c.final_transp(8);
c.final_obs.Ter = c.final_transp(9);
