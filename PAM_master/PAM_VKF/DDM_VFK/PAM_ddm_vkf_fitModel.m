function c = PAM_ddm_vkf_fitModel(u,y,c)
% This is the main function for fitting the parameters of the "PAM_ddm_vkf"
% model
% --------------------------------------------------------------------------------------------------
...
% FIT MODEL
x0 = c.initpt(~c.fix);
fun = @(x) PAM_ddm_vkf(x,u,y,c);
options = optimoptions('fminunc', 'Algorithm', 'quasi-newton');
x = fminunc(fun, x0, options);

c.final = c.initpt;
c.final(~c.fix) = x;
c.final_transp = PAM_ddm_vkf_transp(c.final,y);
...
c.final_obs.a_v = c.final_transp(5);
c.final_obs.b_w = c.final_transp(6);
c.final_obs.b_a = c.final_transp(7);
